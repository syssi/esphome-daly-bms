...
static const uint8_t DALY_FRAME_START_DL_REQ = 0x81;
static const uint8_t DALY_FRAME_START_DL_RESP = 0x51;
...
static const uint16_t DALY_COMMAND_REQ_DL_CELLS_START = 0x0000;
static const uint16_t DALY_COMMAND_REQ_DL_STATUS_START = 0x0041;
static const uint16_t DALY_COMMAND_REQ_DL_ALARMS_START = 0x00A4;
static const uint16_t DALY_COMMAND_REQ_DL_VERSION_START = 0x0178;
static const uint16_t DALY_COMMAND_REQ_DL_SETTINGS1_START = 0x0100;
static const uint16_t DALY_COMMAND_REQ_DL_SETTINGS2_START = 0x0151;
static const uint16_t DALY_COMMAND_REQ_DL_SETTINGS3_START = 0x01C3;
static const uint16_t DALY_COMMAND_REQ_DL_SETTINGS4_START = 0x0220;
static const uint16_t DALY_COMMAND_REQ_DL_SETTINGS5_START = 0x024B;

// DL (0x81) protocol frame data lengths (frame[2] = register count * 2)
static const uint8_t DALY_FRAME_LEN_DL_CELLS = 64 * 2;
static const uint8_t DALY_FRAME_LEN_DL_STATUS = 62 * 2;
static const uint8_t DALY_FRAME_LEN_DL_ALARMS = 10 * 2;
static const uint8_t DALY_FRAME_LEN_DL_VERSION = 74 * 2;
static const uint8_t DALY_FRAME_LEN_DL_SETTINGS1 = 81 * 2;
static const uint8_t DALY_FRAME_LEN_DL_SETTINGS2 = 39 * 2;
static const uint8_t DALY_FRAME_LEN_DL_SETTINGS3 = 80 * 2;
static const uint8_t DALY_FRAME_LEN_DL_SETTINGS4 = 11 * 2;
static const uint8_t DALY_FRAME_LEN_DL_SETTINGS5 = 2 * 2;
...
void DalyBmsBle::update() {
  ...
  if (this->protocol_version_ == DALY_0X81) {
    ...
    this->queue_command_(DALY_FUNCTION_READ, DALY_COMMAND_REQ_DL_VERSION_START, DALY_FRAME_LEN_DL_VERSION / 2);
    this->queue_command_(DALY_FUNCTION_READ, DALY_COMMAND_REQ_DL_SETTINGS1_START, DALY_FRAME_LEN_DL_SETTINGS1 / 2);
    this->queue_command_(DALY_FUNCTION_READ, DALY_COMMAND_REQ_DL_SETTINGS2_START, DALY_FRAME_LEN_DL_SETTINGS2 / 2);
    this->queue_command_(DALY_FUNCTION_READ, DALY_COMMAND_REQ_DL_SETTINGS3_START, DALY_FRAME_LEN_DL_SETTINGS3 / 2);
    this->queue_command_(DALY_FUNCTION_READ, DALY_COMMAND_REQ_DL_SETTINGS4_START, DALY_FRAME_LEN_DL_SETTINGS4 / 2);
    this->queue_command_(DALY_FUNCTION_READ, DALY_COMMAND_REQ_DL_SETTINGS5_START, DALY_FRAME_LEN_DL_SETTINGS5 / 2);
  ...
  if (this->protocol_version_ == DALY_0X81) {
    switch (cmd_address) {
      case DALY_COMMAND_REQ_DL_CELLS_START:
        ...
        break;
      case DALY_COMMAND_REQ_DL_STATUS_START:
        this->decode_dl_status_data_(data);
        break;
      case DALY_COMMAND_REQ_DL_ALARMS_START:
        log_frame_hex(TAG, "[DL] Alarm registers 0x00A4-0x00AD", data);
        break;
      case DALY_COMMAND_REQ_DL_VERSION_START:
        this->decode_dl_version_data_(data);
        break;
      case DALY_COMMAND_REQ_DL_SETTINGS1_START:
        log_frame_hex(TAG, "[DL] Settings registers 0x0100-0x0150", data);
        break;
      case DALY_COMMAND_REQ_DL_SETTINGS2_START:
        log_frame_hex(TAG, "[DL] Settings registers 0x0151-0x0177", data);
        break;
      case DALY_COMMAND_REQ_DL_SETTINGS3_START:
        log_frame_hex(TAG, "[DL] Settings registers 0x01C3-0x0212", data);
        break;
      case DALY_COMMAND_REQ_DL_SETTINGS4_START:
        log_frame_hex(TAG, "[DL] Settings registers 0x0220-0x022A", data);
        break;
      case DALY_COMMAND_REQ_DL_SETTINGS5_START:
        log_frame_hex(TAG, "[DL] Settings registers 0x024B-0x024C", data);
        break;
      case DALY_COMMAND_REQ_BALANCER_SWITCH:
        this->decode_balancer_switch_data_(data);
        break;
      default:
        ESP_LOGW(TAG, "[DL] Unhandled response (addr=0x%04X, len=%zu): %s", cmd_address, data.size(),
                 format_hex_pretty(&data.front(), data.size()).c_str());  // NOLINT
    }
    return;
  }
...
void DalyBmsBle::decode_dl_cells_data_(const std::vector<uint8_t> &data) {
  ...
  ESP_LOGI(TAG, "[DL] RT1: cells=%u temp_sensors=%u", cells, temp_sensors);
  ...
  ESP_LOGI(TAG, "[DL] RT1: %.1fV  %.1fA  SOC=%.1f%%  cells=%u  temps=%u  max_cell=%.3fV  min_cell=%.3fV", total_voltage,
           current, daly_offset_get_16bit(0x3A) * 0.1f, cells, temp_sensors, max_cell_voltage, min_cell_voltage);
}
...
void DalyBmsBle::decode_dl_status_data_(const std::vector<uint8_t> &data) {
  ...
  ESP_LOGI(TAG, "[DL] RT2: status=%s  capacity=%.1fAh  cycles=%u  bal=%u  chg_mos=%u  dis_mos=%u  mosfet_temp=%.0f°C",
           status == 0   ? "Idle"
           : status == 1 ? "Charging"
                         : "Discharging",
...
void DalyBmsBle::decode_dl_version_data_(const std::vector<uint8_t> &data) {
  if (data.size() != DALY_FRAME_LEN_DL_VERSION + 5) {
    ESP_LOGW(TAG, "decode_dl_version_data_: unexpected frame size %zu", data.size());
    return;
  }
  ...
  ESP_LOGI(TAG, "[DL] Software version: %s", software_version.c_str());
  this->publish_state_(this->software_version_text_sensor_, software_version);

  auto hw_begin = data.begin() + 31;
  auto hardware_version = std::string(hw_begin, std::find(hw_begin, hw_begin + 14, '\0'));
  ESP_LOGI(TAG, "[DL] Hardware version: %s", hardware_version.c_str());
  this->publish_state_(this->hardware_version_text_sensor_, hardware_version);
}
